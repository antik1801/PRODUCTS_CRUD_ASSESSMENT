...
const createSemesterRegistrationIntoDB = async (
  ...
  if (isThereAnyUpcomingOrOngoingSemester) {
    throw new AppError(
      ...
      "There is a registered semester"
    );
  }
  ...
  if (currentSemester) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "This current semester is already exists"
    );
  }
...
const updateSemesterRegistrationFromDB = async (
  ...
  if (!isSemesterRegistrationExists) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "This semester registration is not exists in database"
    );
  }
  ...
  if (currentSemesterRegistrationStatus === RegistrationStatus.ENDED) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "This semester registration status is already ended"
    );
  }
  ...
  if (
    ...
    requestSemesterRegistrationStatus === RegistrationStatus.ENDED
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "You can't change any semester registration from UPCOMING to ENDED"
    );
  }
  if (
    currentSemesterRegistrationStatus === RegistrationStatus.ONGOING &&
    requestSemesterRegistrationStatus === RegistrationStatus.UPCOMING
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "You can't change any semester registration status from ONGOING to UPCOMING"
    );
  }
